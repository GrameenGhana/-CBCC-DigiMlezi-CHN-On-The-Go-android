...
public class SessionManager {
    ...
    public static String getUserDisplayName(Context ctx){
        ...
        try {
            CbccUser u = db.getUser(username);
            /*
    ...
    public static void loginUser(Context ctx, CbccUser cbccUser){

        //To ensure that userPrefs get saved, we force the logout of the current cbccUser
        logoutCurrentUser(ctx);

        String username = cbccUser.getUsername();
        SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(ctx);
        SharedPreferences.Editor editor = prefs.edit();
        editor.putString(PrefsActivity.PREF_USER_NAME, username);
        editor.putString(PrefsActivity.PREF_PHONE_NO, cbccUser.getPhoneNo());
        editor.putBoolean(PrefsActivity.PREF_SCORING_ENABLED, cbccUser.isScoringEnabled());
        editor.putBoolean(PrefsActivity.PREF_BADGING_ENABLED, cbccUser.isBadgingEnabled());

        loadUserPrefs(ctx, username, editor);
        setUserApiKeyValid(ctx, cbccUser, true);
         editor.apply();
    }
    ...
    public static void setUserApiKeyValid(Context ctx, CbccUser cbccUser, boolean valid){
        ArrayList<Pair<String, String>> userPrefs = new ArrayList<>();
        Pair<String, String> userPref = new Pair<>(APIKEY_VALID, valid?"true":"false");
        userPrefs.add(userPref);

        DbHelper.getInstance(ctx).insertUserPreferences(cbccUser.getUsername(), userPrefs);
    }
