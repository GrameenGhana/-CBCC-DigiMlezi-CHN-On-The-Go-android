...
public class RegisterFragment extends AppFragment implements SubmitListener {
	...
	public void submitComplete(Payload response) {
		...
		if (response.isResult()) {
			CbccUser cbccUser = (CbccUser) response.getData().get(0);
            SessionManager.loginUser(getActivity(), cbccUser);
	...
	public void onRegisterClick(View view) {
		...
		if (password.length() < org.digitalcampus.oppia.application.MobileLearning.PASSWORD_MIN_LENGTH) {
			UIUtils.showAlert(super.getActivity(),R.string.error,getString(R.string.error_register_password,  org.digitalcampus.oppia.application.MobileLearning.PASSWORD_MIN_LENGTH ));
			return;
		}
